...
两种方式定义的事件处理，`input标签` and `js`。但是实际的操作之后就会发现，控制台只输出了一句话。说明`dom0级`事件处理，**后定义的事件会覆盖前定义的事件**。
...
##  内容分发
内容分发是实现组件开发必不可少的模块。通俗来说，组件的样式内容填充可能不是固定的，这时就需要把带有特殊标记的模块放在组件特定的位置处。`slot`应运而生。
* Props:
    `name`-string,用于命名插槽。
* Usage:
...
* Attributes
     ...
     对外：通过`Attributes`控制组件样式及局部事件触发  
